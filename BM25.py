...
def pivoted(query, fileID):
    ...
    for word in query:
        if word in uniqueWordList:
            ...
            idf = math.log((float(docNumber)-docFreq+0.5)/(docFreq+0.5))
            #tf = (1+math.log(1+math.log(float(termFreq)))) / (1-0.5+0.5*docLength/avgdl)
            tf = (k1+1)*float(termFreq)/(k1*(1-b+b*docLength/avgdl)+float(termFreq))
            qtf = (k3+1)*float(termFreq)/(k3+float(termFreq))
            wordScore = tf * idf * qtf
            score += wordScore
        else:
            score += 0
    return score
